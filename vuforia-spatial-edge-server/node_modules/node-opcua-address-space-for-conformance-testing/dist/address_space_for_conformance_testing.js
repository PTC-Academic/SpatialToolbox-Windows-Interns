...
exports.build_address_space_for_conformance_testing = void 0;
...
function getRandomFuncForType(dataType) {
    ...
    switch (dataTypeName) {
        ...
        case "QualifiedName":
            return () => {
                return new node_opcua_data_model_2.QualifiedName({ name: ec.randomString() });
            };
        case "LocalizedText":
            return () => {
                return new node_opcua_data_model_2.LocalizedText({ text: ec.randomString() });
            };
        case "XmlElement":
            return () => {
...
function _findDataType(dataTypeName) {
    const builtInDataTypeName = node_opcua_factory_1.findBuiltInType(dataTypeName);
...
function validate_value_or_array(isArray, variantValue, validatorFunc) {
    node_opcua_assert_1.assert(typeof validatorFunc === "function");
    let i;
    let value;
    if (isArray) {
...
function makeVariant(dataTypeName, isArray, current_value) {
    ...
    let arrayType = node_opcua_variant_1.VariantArrayType.Scalar;
    if (isArray) {
...
function _add_variable(namespace, parent, varName, dataTypeName, current_value, isArray, extra_name) {
    ...
    node_opcua_assert_1.assert(typeof extra_name === "string");
    ...
    const variable = namespace.addVariable({
        ...
        accessLevel: node_opcua_data_model_1.makeAccessLevelFlag("CurrentRead | CurrentWrite"),
        userAccessLevel: node_opcua_data_model_1.makeAccessLevelFlag("CurrentRead | CurrentWrite"),
        value: variant
    });
...
function add_variable(namespace, parent, name, realType, default_value, extra_name) {
    ...
    node_opcua_assert_1.assert(variable.valueRank === -1);
    // tslint:disable-next-line: no-bitwise
    node_opcua_assert_1.assert(variable.accessLevel === node_opcua_data_model_1.AccessLevelFlag.CurrentRead + node_opcua_data_model_1.AccessLevelFlag.CurrentWrite);
    // tslint:disable-next-line: no-bitwise
    node_opcua_assert_1.assert(variable.userAccessLevel === node_opcua_data_model_1.AccessLevelFlag.CurrentRead + node_opcua_data_model_1.AccessLevelFlag.CurrentWrite);
    node_opcua_assert_1.assert(variable.historizing === false);
    return variable;
}
function add_variable_array(namespace, parent, dataTypeName, default_value, realTypeName, arrayLength, extra_name) {
    ...
    node_opcua_assert_1.assert(typeof realTypeName === "string");
    // istanbul ignore next
    if (!node_opcua_variant_1.DataType[realTypeName]) {
        console.log("dataTypeName", dataTypeName);
        console.log("realTypeName", realTypeName);
    }
    node_opcua_assert_1.assert(node_opcua_variant_1.DataType[realTypeName], " expecting a valid real type");
    arrayLength = arrayLength || 10;
    const local_defaultValue = typeof default_value === "function" ? default_value() : default_value;
    const current_value = node_opcua_variant_1.buildVariantArray(node_opcua_variant_1.DataType[realTypeName], arrayLength, local_defaultValue);
...
const typeAndDefaultValue = [
    ...
    { type: "Int32", defaultValue: 0 },
    {
        type: "NodeId",
        defaultValue() {
            return node_opcua_nodeid_1.coerceNodeId("ns=" + 3 + ";g=00000000-0000-0000-0000-000000000023");
        }
    },
    ...
    { type: "Number", realType: "UInt16", defaultValue: 0 },
    { type: "Integer", realType: "Int64", defaultValue: 0 },
...
function add_simulation_variables(namespace, scalarFolder) {
    ...
    function install_Timer() {
        delete_Timer();
        node_opcua_assert_1.assert(!timer);
        if (enabled) {
            timer = setInterval(() => {
                change_randomly();
            }, interval);
    ...
    intervalVariable.on("value_changed", (dataValue /*,indexRange*/) => {
        ...
        node_opcua_assert_1.assert(ec.isValidUInt16(variant.value), " value must be valid for dataType");
    ...
    enabledVariable.on("value_changed", (dataValue /*,indexRange*/) => {
        const variant = dataValue.value;
        node_opcua_assert_1.assert(variant instanceof node_opcua_variant_1.Variant);
        node_opcua_assert_1.assert(ec.isValidBoolean(variant.value), " value must be valid for dataType");
...
function add_access_right_variables(namespace, parentFolder) {
    ...
    namespace.addVariable({
        ...
        dataType: "Int32",
        valueRank: -1,
        accessLevel: node_opcua_data_model_1.makeAccessLevelFlag("CurrentRead"),
        userAccessLevel: node_opcua_data_model_1.makeAccessLevelFlag("CurrentRead"),
        value: new node_opcua_variant_1.Variant({
    ...
    namespace.addVariable({
        ...
        dataType: "Int32",
        valueRank: -1,
        accessLevel: node_opcua_data_model_1.makeAccessLevelFlag("CurrentWrite"),
        userAccessLevel: node_opcua_data_model_1.makeAccessLevelFlag("CurrentWrite"),
        value: {}
    });
    ...
    namespace.addVariable({
        ...
        valueRank: -1,
        accessLevel: node_opcua_data_model_1.makeAccessLevelFlag("CurrentRead"),
        userAccessLevel: node_opcua_data_model_1.makeAccessLevelFlag(""),
        value: new node_opcua_variant_1.Variant({
            dataType: node_opcua_variant_1.DataType.Int32,
            arrayType: node_opcua_variant_1.VariantArrayType.Scalar,
            value: 36
    ...
    namespace.addVariable({
        ...
        dataType: "Int32",
        valueRank: -1,
        accessLevel: node_opcua_data_model_1.makeAccessLevelFlag("CurrentWrite | CurrentRead"),
        userAccessLevel: node_opcua_data_model_1.makeAccessLevelFlag("CurrentRead"),
        value: new node_opcua_variant_1.Variant({
            dataType: node_opcua_variant_1.DataType.Int32,
            arrayType: node_opcua_variant_1.VariantArrayType.Scalar,
    ...
    namespace.addVariable({
        ...
        dataType: "Int32",
        valueRank: -1,
        accessLevel: node_opcua_data_model_1.makeAccessLevelFlag(""),
        userAccessLevel: node_opcua_data_model_1.makeAccessLevelFlag(""),
        value: new node_opcua_variant_1.Variant({
            dataType: node_opcua_variant_1.DataType.Int32,
            arrayType: node_opcua_variant_1.VariantArrayType.Scalar,
...
function add_analog_data_items(namespace, parentFolder) {
    ...
    function _addAnalogDataItem(localParentFolder, dataType, initialValue) {
        ...
        node_opcua_assert_1.assert(Array.isArray(initialValue) || (initialValue >= engineeringUnitsRange.low && initialValue <= engineeringUnitsRange.high));
...
function add_ObjectWithMethod(namespace, parentFolder) {
    ...
    node_opcua_assert_1.assert(node_opcua_nodeid_1.makeNodeId("MethodNoArgs", namespace.index)
        .toString()
        .match(/s=MethodNoArgs/));
    node_opcua_assert_1.assert(methodNoArgs.nodeId.toString().match(/s=MethodNoArgs/));
    ...
    const methodIO = namespace.addMethod(myObject, {
        ...
        browseName: "MethodIO",
        nodeId: node_opcua_nodeid_1.makeNodeId("MethodIO", namespace.index),
        inputArguments: [
            {
                name: "ShutterLag",
...
function add_enumeration_variable(namespaceDemo, parentFolder) {
    ...
    const e = namespaceDemo.addVariable({
        ...
        value: {
            get() {
                return new node_opcua_variant_1.Variant({ dataType: node_opcua_variant_1.DataType.Int32, value: 1 });
            }
        }
    });
}
function add_trigger_nodes(namespace, parentFolder) {
    ...
    function _add_trigger_node(parent, browseName, nodeId) {
        const triggerNode = namespace.addVariable({
            ...
            dataType: "Double",
            typeDefinition: node_opcua_nodeid_1.makeNodeId(68)
        });
        let value = 100.0;
        const getFunc = () => {
            return new node_opcua_variant_1.Variant({
...
function build_address_space_for_conformance_testing(addressSpace, options) {
    ...
    testHelpers_1.add_eventGeneratorObject(namespace, simulationFolder);
