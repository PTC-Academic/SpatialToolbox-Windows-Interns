...
class ClientSessionImpl extends events_1.EventEmitter {
    ...
    browse(...args) {
        ...
        node_opcua_assert_1.assert(isFinite(this.requestedMaxReferencesPerNode));
    ...
    readAggregateValue(arg0, startTime, endTime, aggregateFn, processingInterval, ...args) {
        ...
        const aggregateFns = Array.isArray(aggregateFn)
            ? aggregateFn
            : [aggregateFn];
        node_opcua_assert_1.assert(aggregateFns.length === nodesToRead.length);
        ...
        node_opcua_assert_1.assert(nodesToRead.length === request.nodesToRead.length);
        this.performMessageTransaction(request, (err, response) => {
            /* istanbul ignore next */
            if (err) {
                return callback(err);
            ...
            response.results = response.results || /* istanbul ignore next */ [];
            node_opcua_assert_1.assert(nodesToRead.length === response.results.length);
            callback(null, isArray ? response.results : response.results[0]);
        });
    }
    /**
    ...
    read(...args) {
        ...
        if (helpAPIChange) {
            ...
            if (!(node_opcua_utils_1.getFunctionParameterNames(callback)[1] === "dataValues" || node_opcua_utils_1.getFunctionParameterNames(callback)[1] === "dataValue")) {
        ...
        if (privateThis.pendingTransactionsCount > 0) {
            ...
            if (privateThis.pendingTransactions.length > 10) {
                if (!pendingTransactionMessageDisplayed) {
                    ...
                    warningLog("[NODE-OPCUA-W21]", "Pending transactions: ", privateThis.pendingTransactions.map((a) => a.request.constructor.name).join(" "));
                    // tslint:disable-next-line: no-console
    ...
    close(...args) {
        ...
        const callback = args[1];
        node_opcua_assert_1.assert(typeof callback === "function");
        node_opcua_assert_1.assert(typeof deleteSubscription === "boolean");
        /* istanbul ignore next */
        if (!this._client) {
    ...
    call(...args) {
        ...
        node_opcua_assert_1.assert(Array.isArray(methodsToCall));
        const callback = args[1];
    ...
    registerNodes(...args) {
        ...
        const callback = args[1];
        node_opcua_assert_1.assert(typeof callback === "function");
        node_opcua_assert_1.assert(Array.isArray(nodesToRegister));
    ...
    unregisterNodes(...args) {
        ...
        const callback = args[1];
        node_opcua_assert_1.assert(typeof callback === "function");
        node_opcua_assert_1.assert(Array.isArray(nodesToUnregister));
    ...
    toString() {
        ...
        str += "\n serverCertificate........ " + node_opcua_utils_1.buffer_ellipsis(this.serverCertificate);
    ...
    extractNamespaceDataType() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!sessionPriv.$$extraDataTypeManager) {
                ...
                yield node_opcua_client_dynamic_extension_object_1.populateDataTypeManager(this, dataTypeManager);
            }
    ...
    recreate_session_and_reperform_transaction(request, callback) {
        ...
        reconnection_1.repair_client_session(this._client, this, (err) => {
            this.recursive_repair_detector -= 1;
            if (err) {
