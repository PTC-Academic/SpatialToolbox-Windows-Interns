...
function __findEndpoint(endpointUrl, params, _callback) {
    ...
    node_opcua_assert_1.assert(!masterClient._tmpClient);
...
function _verify_serverCertificate(certificateManager, serverCertificate, callback) {
    certificateManager.checkCertificate(serverCertificate, (err, status) => {
        ...
        if (status !== node_opcua_status_code_1.StatusCodes.Good) {
            ...
            warningLog("serverCertificate = ", node_opcua_crypto_1.makeSHA1Thumbprint(serverCertificate).toString("hex"));
...
class ClockAdjustment {
    constructor() {
        ...
        node_opcua_date_time_1.installPeriodicClockAdjustment();
    }
    dispose() {
        node_opcua_date_time_1.uninstallPeriodicClockAdjustment();
...
class ClientBaseImpl extends node_opcua_common_1.OPCUASecureObject {
    constructor(options) {
        ...
        if (!options.clientCertificateManager) {
            options.clientCertificateManager = node_opcua_certificate_manager_1.getDefaultCertificateManager("PKI");
        }
        ...
        node_opcua_assert_1.assert(!this.applicationName.match(/urn:/), "applicationName should not be a URI");
        ...
        node_opcua_assert_1.assert(isFinite(this.tokenRenewalInterval) && this.tokenRenewalInterval >= 0);
            ...
            this._internal_create_secure_channel(infiniteConnectionRetry, (err) => {
                if (err) {
                    ...
                    if (true || err.message.match("BadCertificateInvalid")) {
                        ...
                        warningLog("old server certificate ", node_opcua_crypto_1.makeSHA1Thumbprint(this.serverCertificate).toString("hex"));
                        ...
                        return this.fetchServerCertificate(this.endpointUrl, (err1) => {
                            ...
                            warningLog("new server certificate ", node_opcua_crypto_1.makeSHA1Thumbprint(this.serverCertificate).toString("hex"));
    ...
    _internal_create_secure_channel(connectionStrategy, callback) {
        ...
        node_opcua_assert_1.assert(typeof this.endpointUrl === "string");
    ...
    createDefaultCertificate() {
        return __awaiter(this, void 0, void 0, function* () {
            if (!fs.existsSync(this.certificateFile)) {
                ...
                const applicationUri = this._getBuiltApplicationUri();
                const hostname = node_opcua_hostname_1.getHostname();
                // this.serverInfo.applicationUri!;
                yield this.clientCertificateManager.createSelfSignedCertificate({
                    applicationUri,
                    dns: [hostname],
                    // ip: await getIpAddresses(),
                    outputFile: this.certificateFile,
                    subject: node_opcua_certificate_manager_1.makeSubject(applicationName, hostname),
                    startDate: new Date(),
    ...
    _getBuiltApplicationUri() {
        if (!this._applicationUri) {
            this._applicationUri = node_opcua_common_2.makeApplicationUrn(node_opcua_hostname_1.getHostname(), this.applicationName);
        }
    ...
    connect(...args) {
        ...
        node_opcua_assert_1.assert(typeof callback === "function", "expecting a callback");
        ...
        node_opcua_assert_1.assert(typeof endpointUrl === "string" && endpointUrl.length > 0);
        // istanbul ignore next
    ...
    findEndpoint(endpointUrl, securityMode, securityPolicy) {
        ...
        return this._serverEndpoints.find((endpoint) => {
            return (node_opcua_utils_1.matchUri(endpoint.endpointUrl, endpointUrl) &&
    ...
    _removeSession(session) {
        ...
        node_opcua_assert_1.assert(this._sessions.indexOf(session) === -1);
    }
    disconnect(...args) {
        const callback = args[0];
        node_opcua_assert_1.assert(typeof callback === "function", "expecting a callback function here");
        this.reconnectionIsCanceled = true;
    ...
    toString() {
        ...
        str += "  certificate fingerprint....... " + node_opcua_crypto_1.makeSHA1Thumbprint(this.getCertificate()).toString("hex") + "\n";
        str +=
            "  server certificate fingerprint " +
                (this.serverCertificate ? node_opcua_crypto_1.makeSHA1Thumbprint(this.serverCertificate).toString("hex") : "") +
                "\n";
    ...
    _addSession(session) {
        ...
        node_opcua_assert_1.assert(this._sessions.indexOf(session) === -1, "session already added");
                    ...
                    this._recreate_secure_channel((err1) => {
                        ...
                        if (err1) {
                            ...
                            if (this._on_connection_reestablished) {
                                node_opcua_assert_1.assert(typeof this._on_connection_reestablished === "function");
