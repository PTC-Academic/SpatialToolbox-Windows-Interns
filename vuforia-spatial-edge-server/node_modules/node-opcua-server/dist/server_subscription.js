...
function _adjust_maxNotificationsPerPublish(maxNotificationsPerPublish) {
    ...
    node_opcua_assert_1.assert(typeof maxNotificationsPerPublish === "number");
...
function assert_validNotificationData(n) {
    node_opcua_assert_1.assert(n instanceof node_opcua_service_subscription_2.DataChangeNotification || n instanceof node_opcua_service_subscription_2.EventNotificationList || n instanceof node_opcua_service_subscription_2.StatusChangeNotification);
}
...
class Subscription extends events_1.EventEmitter {
    ...
    preCreateMonitoredItem(addressSpace, timestampsToReturn, monitoredItemCreateRequest) {
        ...
        function handle_error(statusCode) {
            return {
                createResult: new node_opcua_types_1.MonitoredItemCreateResult({ statusCode }),
        ...
        node_opcua_assert_1.assert(createResult.statusCode === node_opcua_status_code_1.StatusCodes.Good);
    ...
    notificationHasExpired(notification) {
        ...
        node_opcua_assert_1.assert(isFinite(notification.start_tick + this.maxKeepAliveCount));
    ...
    acknowledgeNotification(sequenceNumber) {
        ...
        let foundIndex = -1;
        this._sent_notifications.forEach((e, index) => {
    ...
    _publish_pending_notifications() {
        ...
        notificationMessage.notificationData.forEach((notificationData) => {
            ...
            if (notificationData instanceof node_opcua_service_subscription_2.DataChangeNotification) {
                ...
                node_opcua_assert_1.assert(notificationData instanceof node_opcua_service_subscription_2.StatusChangeNotification);
                // TODO
        ...
        node_opcua_assert_1.assert(notificationMessage.notificationData.length > 0);
        // update diagnostics
        ...
        node_opcua_assert_1.assert(notificationMessage.sequenceNumber === 0xffffffff);
        ...
        node_opcua_assert_1.assert(!response.notificationMessage ||
    ...
    _tick() {
        ...
        // istanbul ignore next
        if (doDebug) {
            debugLog(t(new Date()) + "  " + this._life_time_counter + "/" + this.lifeTimeCount + chalk.cyan("   Subscription#_tick"), "  processing subscriptionId=", this.id, "hasMonitoredItemNotifications = ", this.hasMonitoredItemNotifications, " publishingIntervalCount =", this.publishIntervalCount);
        }
    ...
    _popNotificationToSend() {
        node_opcua_assert_1.assert(this._pending_notifications.size > 0);
    ...
    _createMonitoredItemStep3(monitoredItem, monitoredItemCreateRequest) {
        ...
        node_opcua_assert_1.assert(typeof monitoredItem.samplingFunc === "function", " expecting a sampling function here");
    ...
    _collectNotificationData() {
        ...
        // reset cache ...
        this._hasMonitoredItemNotifications = false;
        const all_notifications = new queue_1.Queue();
        ...
        while (all_notifications.size > 0) {
            ...
            const dataChangedNotificationData = myFilter(node_opcua_types_1.MonitoredItemNotification, notifications_chunk);
            const eventNotificationListData = myFilter(node_opcua_types_1.EventFieldList, notifications_chunk);
            node_opcua_assert_1.assert(notifications_chunk.length === dataChangedNotificationData.length + eventNotificationListData.length);
            notifications = [];
