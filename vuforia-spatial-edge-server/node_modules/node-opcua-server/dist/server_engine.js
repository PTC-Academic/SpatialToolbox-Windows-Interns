...
function getMonitoredItemsId(inputArguments, context, callback) {
    node_opcua_assert_1.assert(Array.isArray(inputArguments));
    ...
    node_opcua_assert_1.assert(Array.isArray(result.serverHandles));
    node_opcua_assert_1.assert(Array.isArray(result.clientHandles));
...
function __bindVariable(self, nodeId, options) {
    ...
    if (variable && variable.bindVariable) {
        ...
        node_opcua_assert_1.assert(typeof variable.asyncRefresh === "function");
        node_opcua_assert_1.assert(typeof variable.refreshFunc === "function");
    }
    else {
...
class ServerEngine extends events_1.EventEmitter {
    ...
    registerShutdownTask(task) {
        node_opcua_assert_1.assert(typeof task === "function");
    ...
    setShutdownReason(reason) {
        ...
        (_a = this.addressSpace) === null || _a === void 0 ? void 0 : _a.rootFolder.objects.server.serverStatus.shutdownReason.setValueFromSource({
            ...
            value: node_opcua_data_model_1.coerceLocalizedText(reason)
        });
    }
    /**
    ...
    initialize(options, callback) {
        node_opcua_assert_1.assert(!this.addressSpace); // check that 'initialize' has not been already called
        ...
        nodeJS_1.generateAddressSpace(this.addressSpace, options.nodeset_filename, () => {
            if (!this.addressSpace) {
            ...
            // of namespaces
            const namespaces = node_opcua_nodeid_1.makeNodeId(node_opcua_constants_1.ObjectIds.Server_Namespaces);
            ...
            const bindStandardScalar = (id, dataType, func, setter_func) => {
                node_opcua_assert_1.assert(typeof id === "number", "expecting id to be a number");
                node_opcua_assert_1.assert(typeof func === "function");
                node_opcua_assert_1.assert(typeof setter_func === "function" || !setter_func);
            ...
            const bindStandardArray = (id, variantDataType, dataType, func) => {
                ...
                bindVariableIfPresent(nodeId, {
                    get() {
                        const value = func();
                        node_opcua_assert_1.assert(Array.isArray(value));
                        return new node_opcua_variant_1.Variant({
                            arrayType: node_opcua_variant_1.VariantArrayType.Array,
            ...
            const bindServerCapabilities = () => {
                ...
                const bindOperationLimits = (operationLimits) => {
                    node_opcua_assert_1.assert(operationLimits !== null && typeof operationLimits === "object");
                ...
                function fix_ProgramStateMachineType_ProgramDiagnostics() {
                    const nodeId = node_opcua_nodeid_1.coerceNodeId("i=2399"); // ProgramStateMachineType_ProgramDiagnostics
            ...
            const bindExtraStuff = () => {
                ...
                const namingRuleDataTypeNode = addressSpace.findDataType(node_opcua_nodeid_1.resolveNodeId(node_opcua_constants_1.DataTypeIds.NamingRuleType));
            ...
            bindExtraStuff();
            this.__internal_bindMethod(node_opcua_nodeid_1.makeNodeId(node_opcua_constants_1.MethodIds.Server_GetMonitoredItems), getMonitoredItemsId.bind(this));
            ...
            const fixGetMonitoredItemArgs = () => {
                ...
                node_opcua_assert_1.assert(dataValue.value.value[0].arrayDimensions.length === 1 && dataValue.value.value[0].arrayDimensions[0] === 0);
                node_opcua_assert_1.assert(dataValue.value.value[1].arrayDimensions.length === 1 && dataValue.value.value[1].arrayDimensions[0] === 0);
            };
            ...
            const prepareServerDiagnostics = () => {
                ...
                if (true) {
                    ...
                    // TO DO ...
                    serverDiagnosticsNode.enabledFlag.userAccessLevel = node_opcua_data_model_1.makeAccessLevelFlag("CurrentRead");
                    serverDiagnosticsNode.enabledFlag.accessLevel = node_opcua_data_model_1.makeAccessLevelFlag("CurrentRead");
                }
                ...
                node_opcua_assert_1.assert(subscriptionDiagnosticsArrayNode.nodeClass === node_opcua_data_model_1.NodeClass.Variable);
                node_opcua_address_space_1.bindExtObjArrayNode(subscriptionDiagnosticsArrayNode, "SubscriptionDiagnosticsType", "subscriptionId");
            ...
            for (const browseDescription of nodesToBrowse) {
                ...
                if (node) {
                    if (node.onFirstBrowseAction) {
                        ...
                        node_opcua_assert_1.assert(node.onFirstBrowseAction === undefined, "expansion can only be made once");
                    }
                }
            }
    ...
    readSingleNode(context, nodeId, attributeId, timestampsToReturn) {
        context.currentTime = node_opcua_date_time_1.getCurrentClock();
    ...
    read(context, readRequest) {
        ...
        node_opcua_assert_1.assert(Array.isArray(nodesToRead));
        context.currentTime = node_opcua_date_time_1.getCurrentClock();
        const dataValues = [];
    ...
    write(context, nodesToWrite, callback) {
        ...
        const performWrite = (writeValue, inner_callback) => {
            node_opcua_client_dynamic_extension_object_1.resolveDynamicExtensionObject(writeValue.value.value, l_extraDataTypeManager);
        ...
        node_opcua_address_space_1.ensureDatatypeExtractedWithCallback(this.addressSpace, (err2, extraDataTypeManager) => {
            ...
            async.map(nodesToWrite, performWrite, (err, statusCodes) => {
                node_opcua_assert_1.assert(Array.isArray(statusCodes));
    ...
    createSession(options) {
        ...
        node_opcua_assert_1.assert(typeof sessionTimeout === "number");
    ...
    closeSession(authenticationToken, deleteSubscriptions, reason) {
        ...
        node_opcua_assert_1.assert(typeof reason === "string");
    ...
    transferSubscription(session, subscriptionId, sendInitialValues) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            node_opcua_assert_1.assert(typeof subscriptionId === "number");
            node_opcua_assert_1.assert(typeof sendInitialValues === "boolean");
            if (subscriptionId <= 0) {
            ...
            if (!sessions_compatible_for_transfer_1.sessionsCompatibleForTransfer(subscription.$session, session)) {
    ...
    _readSingleNode(context, nodeToRead, timestampsToReturn) {
        ...
        timestampsToReturn = node_opcua_data_value_1.coerceTimestampsToReturn(timestampsToReturn);
        const obj = this.__findNode(nodeId);
        let dataValue;
        if (!obj) {
            ...
            dataValue = node_opcua_data_value_1.apply_timestamps_no_copy(dataValue, timestampsToReturn, attributeId);
            return dataValue;
        }
    }
    ...
    __internal_bindMethod(nodeId, func) {
        ...
        if (methodNode && methodNode.bindMethod) {
            ...
            console.log(node_opcua_debug_1.traceFromThisProjectOnly());
        }
    }
    _getServerSubscriptionDiagnosticsArrayNode() {
        ...
        const subscriptionDiagnosticsArrayNode = this.addressSpace.findNode(node_opcua_nodeid_1.makeNodeId(node_opcua_constants_1.VariableIds.Server_ServerDiagnostics_SubscriptionDiagnosticsArray));
