...
function _adjust_sampling_interval(samplingInterval, node_minimumSamplingInterval) {
    node_opcua_assert_1.assert(typeof node_minimumSamplingInterval === "number", "expecting a number");
...
function _validate_parameters(monitoringParameters) {
    ...
    node_opcua_assert_1.assert(isFinite(monitoringParameters.clientHandle));
    node_opcua_assert_1.assert(isFinite(monitoringParameters.samplingInterval));
    node_opcua_assert_1.assert(typeof monitoringParameters.discardOldest === "boolean");
    node_opcua_assert_1.assert(isFinite(monitoringParameters.queueSize));
...
function valueHasChanged(newDataValue, oldDataValue, deadbandType, deadbandValue) {
    ...
    switch (deadbandType) {
        case node_opcua_service_subscription_2.DeadbandType.None:
            ...
            return node_opcua_service_subscription_2.isOutsideDeadbandNone(oldDataValue.value, newDataValue.value);
        case node_opcua_service_subscription_2.DeadbandType.Absolute:
            // AbsoluteDeadband
            return node_opcua_service_subscription_2.isOutsideDeadbandAbsolute(oldDataValue.value, newDataValue.value, deadbandValue);
        default:
            ...
            if (euRangeNode && euRangeNode.nodeClass === node_opcua_data_model_1.NodeClass.Variable) {
                ...
                return node_opcua_service_subscription_2.isOutsideDeadbandPercent(oldDataValue.value, newDataValue.value, deadbandValue, rangeVariant.value);
            }
            else {
...
function apply_filter(newDataValue) {
    ...
    if (this.filter instanceof node_opcua_service_subscription_2.DataChangeFilter) {
        ...
        return !node_opcua_variant_1.sameVariant(newDataValue.value, this.oldDataValue.value);
    }
    return true; // keep
    // else {
...
class MonitoredItem extends events_1.EventEmitter {
    constructor(options) {
        ...
        this.timestampsToReturn = node_opcua_data_value_1.coerceTimestampsToReturn(options.timestampsToReturn);
    ...
    recordValue(dataValue, skipChangeTest, indexRange) {
        ...
        // istanbul ignore next
        if (doDebug) {
            debugLog("MonitoredItem#recordValue", this.node.nodeId.toString(), this.node.browseName.toString(), " has Changed = ", !node_opcua_data_value_1.sameDataValue(dataValue, this.oldDataValue), "skipChangeTest = ", skipChangeTest, "hasSemanticChanged = ", hasSemanticChanged);
        }
    ...
    extractMonitoredItemNotifications() {
        ...
        if (notifications.length > 0 && this.node && this._semantic_version < this.node.semantic_version) {
            ...
            setSemanticChangeBit(dataValue);
            node_opcua_assert_1.assert(this.node.nodeClass === node_opcua_data_model_1.NodeClass.Variable);
            this._semantic_version = this.node.semantic_version;
        }
    ...
    _stop_sampling() {
        ...
        if (this._on_opcua_event_received_callback) {
            node_opcua_assert_1.assert(typeof this._on_opcua_event_received_callback === "function");
            this.node.removeListener("event", this._on_opcua_event_received_callback);
            this._on_opcua_event_received_callback = null;
        }
        if (this._attribute_changed_callback) {
            node_opcua_assert_1.assert(typeof this._attribute_changed_callback === "function");
        ...
        if (this._value_changed_callback) {
            ...
            node_opcua_assert_1.assert(typeof this._value_changed_callback === "function");
            node_opcua_assert_1.assert(!this._samplingId);
        ...
        if (this._semantic_changed_callback) {
            node_opcua_assert_1.assert(typeof this._semantic_changed_callback === "function");
            node_opcua_assert_1.assert(!this._samplingId);
    ...
    _on_opcua_event(eventData) {
        ...
        if (!node_opcua_address_space_1.checkWhereClause(addressSpace, node_opcua_address_space_1.SessionContext.defaultContext, this.filter.whereClause, eventData)) {
            return;
        }
        const selectClauses = this.filter.selectClauses ? this.filter.selectClauses : [];
        const eventFields = node_opcua_address_space_1.extractEventFields(node_opcua_address_space_1.SessionContext.defaultContext, selectClauses, eventData);
        // istanbul ignore next
        if (doDebug) {
    ...
    _setOverflowBit(notification) {
        if (notification.hasOwnProperty("value")) {
            ...
            node_opcua_assert_1.assert(node_opcua_data_value_1.sameStatusCode(notification.value.statusCode, node_opcua_status_code_1.StatusCodes.GoodWithOverflowBit));
    ...
    _makeEventFieldList(eventFields) {
        node_opcua_assert_1.assert(Array.isArray(eventFields));
